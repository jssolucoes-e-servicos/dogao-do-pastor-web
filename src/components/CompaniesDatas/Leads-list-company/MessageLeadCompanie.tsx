...
export default function MessageIntentsCompanie({
  ...
  const [selectedIntentColor, setSelectedIntentColor] = useState<Lead | null>(null);
  ...
  // Função para filtrar os leads pelo termo de pesquisa e status
  const filteredLeads = leadsData?.leads?.filter((lead: Lead) => {
    return (
      lead.phone.includes(searchTerm) && // Filtra pelo telefone
      (filteredStatus === "" || lead.status === filteredStatus) // Filtra pelo status
    );
  });
...
<div className="col-span-12 px-2 relative overflow-y-auto border-stroke dark:bg-neutral-800/50  dark:bg-aibitMenu xl:col-span-1
    mb-2">
   {/* Campo de entrada para pesquisa pelo telefone */}
   <input
              type="text"
              placeholder="Pesquisar pelo telefone"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="dark:bg-rose-700 bg-rose-700 text-white py-2 px-2 rounded text-[14px] w-full outline-none placeholder-text placeholder-gray-50"
            />
</div>
        ...
        {!selectedIntent && (
          <div
            className="col-span-12 flex flex-col justify-center items-center h-screen min-h-screen max-h-screen p-7.5 shadow-default dark:border-strokedark xl:col-span-8 relative shadow-lg pb-4"
          >
...
const IntentDetails = ({
  ...
  return (
    <div
      className="col-span-12  h-screen min-h-screen  max-h-screen dark:bg-neutral-800 bg-white p-7.5 shadow-default dark:border-strokedark  xl:col-span-8 relative
     shadow-lg  pb-4"
    >
      {isLoading && (
        <div
                      ...
                      {formatTimestamp(item.timestamp)}
                          ...
                          <>
                            <img
                              ...
                              className="h-auto w-auto rounded-tl-[15px] rounded-bl-[15px] rounded-br-[15px]"
                            />
                            <span className="dark:text-white text-neutral-500 text-sm py-3">
                              Localização
                            </span>
                            <span className="dark:text-white text-neutral-500 text-sm">
                              {item.message.location.name}
                            </span>
                            <span className="dark:text-white text-neutral-500 text-sm max-w-64">
                              {item.message.location.address}
                            </span>
                          </>
      ...
      {(userData.profile === "master" || userData.profile === "manager") && (
        <>
          {lead && Array.isArray(lead.dialog) && (
            // <div
            //   // max-w-[1990px]
            //   className="flex flex-row items-center justify-between fixed z-10  pl-2 gap-2 w-full  bg-white dark:bg-[#6c3eca] py-2 bottom-0"
            //   style={{ zIndex: "99999!important" }}
            // >
            <div className="flex flex-row items-center justify-between relative bottom-5 z-40 px-6 py- h-auto w-full dark:bg-transparent bg-transparent">
              ...
              <div className="flex justify-center items-center relative h-11 w-full min-w-[200px]">
                  <textarea
                    ...
                    className="peer h-10 max-h-12 w-full overflow-hidden rounded-md border border-blue-gray-200 bg-transparent px-3 py-3 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-rose-700 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50"
                    placeholder=" "
                  />
